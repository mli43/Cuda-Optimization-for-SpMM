...
template <typename T> class SparseMatrixCSR : public SparseMatrix<T> {
  public:
    ...
    SparseMatrixCSR(Matrix::metadataType numRows, Matrix::metadataType numCols, Matrix::metadataType numNonZero, bool onDevice) {
    ...
    SparseMatrixCSR<T>* copy2Device() {
        ...
        SparseMatrixCSR<T>* newMatrix = new SparseMatrixCSR<T>(this->numRows, this->numCols, this->numNonZero, true);

        cudaCheckError(cudaMemcpy(newMatrix->rowPtrs, this->rowPtrs,
                                  (this->numRows + 1) *
    ...
    bool allocateSpace(bool onDevice) {
        ...
        if (onDevice) {
            cudaCheckError(cudaMalloc(
                &this->data, this->numNonZero * sizeof(T)));
            cudaCheckError(cudaMalloc(&this->rowPtrs, (this->numRows + 1) * sizeof(Matrix::metadataType)));
            cudaCheckError(cudaMalloc(&this->colIdxs, this->numNonZero * sizeof(Matrix::metadataType)));
            cudaCheckError(cudaMemset(this->data, 0, this->numNonZero * sizeof(T)));
            cudaCheckError(cudaMemset(this->rowPtrs, 0, (this->numRows + 1) * sizeof(Matrix::metadataType)));
            cudaCheckError(cudaMemset(this->colIdxs, 0, this->numNonZero * sizeof(Matrix::metadataType)));
        } else {
            cudaCheckError(cudaMallocHost(
                &this->data, this->numNonZero * sizeof(T)));
            cudaCheckError(cudaMallocHost(&this->rowPtrs, (this->numRows + 1) * sizeof(Matrix::metadataType)));
            cudaCheckError(cudaMallocHost(&this->colIdxs, this->numNonZero * sizeof(Matrix::metadataType)));
            std::memset(this->data, 0, this->numNonZero * sizeof(T));
            std::memset(this->rowPtrs,0, (this->numRows + 1) * sizeof(Matrix::metadataType));
            std::memset(this->colIdxs, 0, this->numNonZero * sizeof(Matrix::metadataType));
        }
    ...
    DenseMatrix<T>* toDense() {
        ...
        DenseMatrix<T>* dm = new DenseMatrix<T>(this->numRows, this->numCols, false);
