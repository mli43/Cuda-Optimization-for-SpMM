...
int main(int argc, char *argv[]) {
    ...
    cudaCheckError(cudaSetDevice(7));

    testcase = input_dirname;

    bool is_large = (input_dirname.rfind("../data/large", 0) == 0);
    is_large = false;
    ...
    if (TEST_COO) {
        ...
        cuspmm::runEngine(engine, a, dense, abs_tol, rel_tol, is_large);
    }
